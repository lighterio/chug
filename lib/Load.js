...
var AssetTree = require('./AssetTree');
...
var Load = module.exports = Waiter.extend({
	...
	init: function init(location, parent) {
		...
		self.changePath = '';
		self.ignoreList = [];
		self.sortTree = new AssetTree();
		if (location) {
			self.add(location);
		}
		self.then(function () {
			// Put manually-added assets at the end.
			self.assets.forEach(function (asset) {
				asset.sortIndex = 1e6;
			});
			// Traverse the tree, populating sortIndex values.
			var sortCount = 0;
			function populateSortIndex(tree) {
				if (tree.data) {
					tree.data.sortIndex = ++sortCount;
				}
				if (tree.children) {
					tree.children.forEach(function (node) {
						populateSortIndex(node);
					});
				}
			}
			populateSortIndex(self.sortTree);
			// Sort the assets.
			self.assets = self.assets.sort(function (a, b) {
	 ...
	 */
	add: function add(location, sortTree) {
		var self = this;
		sortTree = sortTree || self.sortTree;
		if (location instanceof Array) {
			location.forEach(function addLocation(location) {
				self.add(location, sortTree.add());
			});
		}
	 ...
	 */
	addFile: function addFile(path, dirDepth, sortTree) {
		var self = this;
		self.wait();
		fs.stat(path, function statFile(err, stat) {
			...
			if (stat.isDirectory()) {
				self.addDir(path, dirDepth, sortTree, stat);
			} else {
				self.addAsset(File, path, sortTree, stat);
			};
			if (stat.isDirectory() || dirDepth == 0) {
	 ...
	 */
	addDir: function addDir(dir, dirDepth, sortTree, stat) {
		var self = this;
		self.wait();
		fs.readdir(dir, function processDir(err, files) {
			...
			files.forEach(function processFile(name) {
				...
				if (!shouldIgnore) {
					var path = dir + '/' + name;
					self.addFile(path, dirDepth + 1, sortTree.add());
				}
			});
			self.unwait();
		});
	 ...
	 */
	addAsset: function addAsset(assetType, location, sortTree, stat) {
		var self = this;
		chug = require('../chug');
		...
		(sortTree || self.sortTree).data = asset;
		return asset;
	},
	...
	concat: function(location, load) {
		...
		self.then(function thenConcat() {
			...
			if (load.assets.length < 1) {
				load.addAsset(Asset, location, load.sortTree);
			}
