...
// Load chug after chug has loaded "Load".
var chug;
function getChug() {
  chug = chug || require('../chug');
  return chug;
}
...
var Load = module.exports = Waiter.extend({
  ...
  add: function add(location) {
    ...
    if (location instanceof Array) {
      ...
      var wildCard = location.indexOf('*');
      if (wildCard >= 0) {
        self.filter(path);
        path = path.substr(0, wildCard).replace(/[\/\\]$/, '');
      }
      if (!self.isReady) {
  ...
  addFile: function addFile(path, dirDepth) {
    ...
    fs.stat(path, function statFile(err, stat) {
      if (err) {
        self.unwait();
        getChug()._logger.error('Could not stat file: ' + path, err);
        return;
      }
