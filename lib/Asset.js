...
var Asset = module.exports = Waiter.extend({
	...
	compile: function compile() {
		...
		var compiler = api._compilers[this.type];
	...
	minify: function minify() {
		...
		// Get the compiler for this asset's file type.
		var minifier = api._minifiers[this.type];
	...
	route: function route() {
		...
		if (api._app) {
			...
			api._app.get(self.location, function (request, response) {
				response.setHeader('Content-Type', mimeType);
				response.statusCode = 200;
				response.end(self.getShrunkenContent());
			});
		}
		else {
			api.error('Cannot route until setApp has received an Express-style app.');
		}
		return self;
	}
