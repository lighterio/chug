	...
	compile: function compile() {
		...
		if (compiler) {
			...
			if (isFunction(compiler.compile)) {
				...
				chug._error('Unrecognized compiler for type: ' + this.type);
			}
		...
		if (chug._app) {
			...
			function handle(request, response) {
				...
				if (typeof content == 'function') {
					...
					context.revisionTag = chug._app._revisionTag;
			...
			chug._app.get(url, function (request, response) {
				handle(request, response);
			});

			var urlPattern = url.replace(/\.([a-z]+)$/, '.*.$1');
			chug._app.get(urlPattern, function (request, response) {
			...
			chug._error('Cannot route until setApp has received an Express-style app.');
		}
		return self;
	},
